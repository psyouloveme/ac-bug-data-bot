...
})({"fC9pI":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = 1234;
...
function bugCallback(nodecg, bug) {
    ...
    if (bug) {
        ...
        if (bug.available !== bug.peak) outMsg += ` peaks in ${bug.peak}.`;
        outMsg += ` ${bug.spawn} spawn rate (in this route)`;
        if (bug.rain) outMsg += ".";
        else outMsg += " but not in rain.";
...
function setupBugData(nodecg, sqlClient) {
    ...
    nodecg.listenFor("ChatReceived", "twitch-listener", ({ user , message  })=>{
        if (!user || !message || !message.startsWith("!bug ")) return;
        ...
        gameVersion.value;
        sqlClient("ac_bug_data").first("name", "available", "peak", "time", "location", "spawn", "rain").where("name", bugName).then((row)=>{
            if (row) bugCallback(nodecg, row);
            else nodecg.log.info("no bug found,", bugName);
        });
    });
...
},{}]},["fC9pI","3fArG"], "3fArG", "parcelRequireb718")
